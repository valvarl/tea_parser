...
import { RxChevronDown, RxTriangleRight, RxTriangleDown } from "react-icons/rx";
import { FiClock, FiCalendar, FiType, FiDollarSign, FiStar } from "react-icons/fi";

/* ——— скрываемые характеристики ——— */
const HIDDEN_CHAR_IDS = new Set(["Sku", "Brand", "ServiceMaxTemperature", "ServiceMinTemperature", "ShelfLife"]);
...
export default function ProductsFilter({
  ...
  disabled = false,
  loadingFacets = false,
  onSearchFocusChange, // NEW: сообщаем родителю о фокусе
  onSearchCommit, // NEW: просим родителя «записать URL сейчас»
  onSearchTyping, // NEW: отдаём текущее значение ввода при каждом наборе
}) {
  ...
  // синхронизируем из props только когда поле не в фокусе
  useEffect(() => {
    if (!isFocused) setSearchLocal(value?.q || "");
  }, [value?.q, isFocused]);
  ...
  // (1) ДЕБАУНС-ПОИСК: через 400 мс бездействия применяем q, не теряя фокус
  useEffect(() => {
    if (isComposing) return; // не триггерим во время IME-композиции
    const t = setTimeout(() => {
  ...
  }, [searchLocal, isComposing, onChange, value]);
  ...
  const SortTrigger = (
    <button
      ...
      className="form-input input-md w-full pl-2 pr-9 flex items-center justify-between"
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    // (1) Enter — убираем фокус и применяем поиск немедленно
                    e.currentTarget.blur();
                    onSearchCommit?.(); // попросим родителя обновить URL
                  }
                }}
                onFocus={() => {
                  setIsFocused(true);
                ...
                onBlur={() => {
                  setIsFocused(false);
                  onSearchFocusChange?.(false); // родитель поймёт, что можно писать URL
                }}
            ...
            {/* Кнопку Search удалили: авто-применение по дебаунсу 400 мс */}
          </div>
        </div>
              ...
              <ul
                ...
                {SORT_OPTIONS.map((o) => (
                  <li
                    ...
                    className={`px-2 py-2 rounded cursor-pointer flex items-center gap-2 hover:bg-gray-100 ${
                      o.key === currentSortKey ? "bg-gray-50" : ""
                    }`}
                    onClick={() => {
                      updateSort(o.key);
                      setSortOpen(false);
        ...
        {/* Scope — высота ровно как у input-md (42px) */}
        {mode === "byTask" ? (
          <div className="w-full md:w-48 shrink-0">
            <label className="form-label">Scope</label>
            <div
              className="flex rounded-lg overflow-hidden border"
              style={{ height: 42 }} // строго 42px, как у .input-md
            >
              <button
                type="button"
              ...
              <button
                type="button"
                className={`flex-1 px-3 text-sm border-l ${
      ...
      {chips.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {chips.map((ch) => (
            <span key={ch.id} className="px-2 py-1 bg-blue-50 text-blue-700 rounded-full text-xs flex items-center gap-1">
              {ch.label}
              <button
                type="button"
          ...
          <button type="button" onClick={clearAll} className="px-2 py-1 bg-gray-100 text-gray-700 rounded-full text-xs hover:bg-gray-200">
            Clear all
          </button>
        </div>
      )}
      ...
      <details className="mt-3" open={filtersOpen} onToggle={(e) => setFiltersOpen(e.currentTarget.open)}>
        ...
        {/* ——— СТАБИЛЬНАЯ GRID-СЕТКА: карточки не «перескакивают» между колонками ——— */}
        <div className="mt-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {/* skeleton для фасетов */}
          {loadingFacets &&
            Array.from({ length: 6 }).map((_, i) => (
              <div key={`facet-sk-${i}`} className="bg-white border rounded-lg p-3">
              ...
              return (
                <div key={c.id} className="bg-white border rounded-lg">
                  <button
                    ...
                    <span className="text-gray-500 inline-flex items-center">{isOpen ? <RxTriangleDown /> : <RxTriangleRight />}</span>
                  </button>

                  <div id={`char-panel-${c.id}`} className={`transition-all ${isOpen ? "max-h-[480px]" : "max-h-0"} overflow-hidden`}>
                    <div className="p-3">
                        ...
                        {values.map((v) => {
                          ...
                          return (
                            <label key={v} className="inline-flex items-center gap-2 text-sm">
                              <input type="checkbox" checked={checked} onChange={() => toggleFilterValue(c.id, v)} disabled={isDisabled} />
                              <span className="text-gray-800 truncate">{v}</span>
                            </label>
                          );
                        })}
                        {allValues.length === 0 && <div className="text-xs text-gray-500">No values</div>}
                      </div>
                      ...
                      {hasMore && (
                        <div className="mt-2">
                          <button
                            ...
                            onClick={() => setShowCountByChar((s) => ({ ...s, [c.id]: (s[c.id] || INITIAL_SHOW) + SHOW_STEP }))}>
                            Показать ещё
                          </button>
                          <button
                            type="button"
...
ProductsFilter.propTypes = {
  ...
  onSearchFocusChange: PropTypes.func,
